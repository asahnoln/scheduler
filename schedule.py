...
class BusyRange:
    ...
    def __init__(self, from_time: TimeType, to_time: TimeType) -> None:
        self.from_time = from_time if isinstance(from_time, time) else time.fromisoformat(from_time)
        self.to_time = to_time if isinstance(to_time, time) else time.fromisoformat(to_time)
...
class Schedule:
    ...
    def add(self, schedule: Schedule) -> Schedule:
        ...
        for key, busy_range in enumerate(all_ranges):
            ...
            if busy_range.to_time < all_ranges[key + 1].from_time:
                new_ranges.append(BusyRange(new_from_time, busy_range.to_time))
                new_from_time = None

        new_schedule = Schedule(new_ranges)
        return new_schedule
